...
public class KOnkeyDeleteMailMsg implements GameMessageProcesser, KMailProtocol {

	@Override
	public GameMessageProcesser newInstance() {
		return new KOnkeyDeleteMailMsg();
	}

	@Override
	public int getMsgIdHandled() {
		return CM_ONEKEY_DELETE_MAIL;
	}
	...
	public void processMessage(KGameMessageEvent msgEvent) throws Exception {
		...
		result = KMailLogic.dealMsg_oneKeyDeleteMails(role);
		doFinally(session, role, result);
	}

	private void doFinally(KGamePlayerSession session, KRole role, MailResult_Sync result) {
		KGameMessage backMsg = KGame.newLogicMessage(SM_ONEKEY_DELETE_MAIL_RESULT);
