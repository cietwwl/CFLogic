...
import com.kola.kmp.logic.map.OtherEntityStateChangedMsgPacker;
 ...
 * </pre>
 */
public class DuplicateBroadcastMapEntityStateTimerTask implements
		KGameTimerTask {

	@Override
	public String getName() {
		return DuplicateBroadcastMapEntityStateTimerTask.class.getName();
	}
	...
	public Object onTimeSignal(KGameTimeSignal timeSignal)
		...
		Map<Integer, KDuplicateMapImpl> allMaps = KDuplicateMapManager
				.getInstace().getAllDuplicateMaps();

		if (allMaps != null) {
			// 遍历所有地图
			for (KDuplicateMapImpl map : allMaps.values()) {
				if (map.unitLimitType != KDuplicateMapImpl.UNIT_LIMIT_TYPE_NOT_SHOW) {
					// 遍历该地图中所有的角色
					for (KGameMapEntity roleEntity : map
							.getEntities(KMapEntityTypeEnum.ENTITY_TYPE_PLAYERROLE)) {
						if (roleEntity != null) {
							KRole role = (KRole) roleEntity.getSourceObject();
							if (role != null) {
								if (!role.isFighting() && role.isOnline()) {
									// 获取打包数据消息,并发送
									KGameMessage msg = roleEntity
											.getOtherEntityStateChangeDataMessage(
													currentTimeMillis,
													OtherEntityStateChangedMsgPacker.PACK_MSG_TYPE_GAME_MAP);
									if (msg != null) {
										role.sendMsg(msg);
									}
								}
