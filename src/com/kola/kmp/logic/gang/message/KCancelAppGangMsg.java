...
public class KCancelAppGangMsg implements GameMessageProcesser, KGangProtocol {
	@Override
	public GameMessageProcesser newInstance() {
		return new KCancelAppGangMsg();
	}

	@Override
	public int getMsgIdHandled() {
		return CM_GANG_CANCEL_APP_GANG;
	}
	...
	public void processMessage(KGameMessageEvent msgEvent) throws Exception {
		...
		// 军团--取消申请加入军团
		GangResultExt result = KGangLogic.dealMsg_cancelAppGang(role, gangId);
		dofinally(session, role, gangId, result);
	}

	private void dofinally(KGamePlayerSession session, KRole role, long gangId, GangResultExt result) {
		KGameMessage msg = KGame.newLogicMessage(SM_GANG_CANCEL_APP_GANG_RESULT);
